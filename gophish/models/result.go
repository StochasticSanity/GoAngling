...
// HandleSessionCaptured updates a Result in the case where they submitted TFA 
// Tokens to the form on the proxy landing.
func (r *Result) HandleSessionCaptured(details EventDetails) error {
	event, err := r.createEvent(EventSessionCaptured, details)
	if err != nil {
		return err
	}
	r.Status = EventSessionCaptured
	r.ModifiedDate = event.Time
	return db.Save(r).Error
}
...
func generateResultId() (string, error) {
	...
	// Increase length of User IDs - Source: https://github.com/fin3ss3g0d/evilgophish/blob/main/gophish/models/result.go
	k := make([]byte, 12)
	for i := range k {
