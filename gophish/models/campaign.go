...
type Campaign struct {
	...
	PageId        int64     `json:"-"`
	// Removed since we no longer have landing pages
	// Page          Page      `json:"page"`
...
type CampaignStats struct {
	...
	SessionCaptured int64 `json:"session_captured"`
...
func getCampaignStats(cid int64) (CampaignStats, error) {
	...
	query.Where("status=?", EventSessionCaptured).Count(&s.SessionCaptured)
	if err != nil {
		return s, err
	}
	...
	// Every session captured event implies they submitted data
	s.SubmittedData += s.SessionCaptured
...
func PostCampaign(c *Campaign, uid int64) error {
	...
	// Removed since we don't have landing pages anymore
