...
const save = id => {
    ...
    if (id != -1) {
        user.id = id;
        api.userId.put(user).success(data => {
        ...
        api.users.post(user).success(data => {
...
const edit = id => {
    ...
    if (id == -1) {
        ...
        api.userId.get(id).success(user => {
...
const deleteUser = id => {
    ...
    Swal.fire({
        ...
        if (result.value) {
            Swal.fire("User Deleted!", "The user account for " + escapeHtml(user.username) + " and all associated objects have been deleted!", "success")
        }
