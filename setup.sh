...
function install_dependencies() {
    ...
    sudo apt-get install -y build-essential openssl wget git tmux net-tools nginx-full certbot
...
function setup_nginx() {
    ...
    # If redirect is true, set server_name to subdomains
    if [[ $(echo "${redirect}" | grep -ci "true") -gt 0 ]]; then
        server_name=$formatted_subdomains
    # If redirect is false, set server_name to main domain and subdomains
    else
        server_name="${domain} ${formatted_subdomains}"
    fi
...
function setup_apache() {
    ...
    sed -i "s|SSLCertificateFile|SSLCertificateFile /etc/letsencrypt/live/${domain}/cert.pem|g" config/apache/000-default.conf
    sed -i "s|SSLCertificateChainFile|SSLCertificateChainFile /etc/letsencrypt/live/${domain}/fullchain.pem|g" config/apache/000-default.conf
    sed -i "s|SSLCertificateKeyFile|SSLCertificateKeyFile /etc/letsencrypt/live/${domain}/privkey.pem|g" config/apache/000-default.conf
...
function update_apache_conf() {
    info "Moving new configuration files..."
    cp config/apache/000-default.conf /etc/apache2/sites-enabled/
