...
func main() {
	...
	if *cfg_dir == "" {
		// Finally fixed this - always bugged me.
		...
		*cfg_dir = joinPath(exe_dir, "./config")
		if _, err := os.Stat(*cfg_dir); os.IsNotExist(err) {
			*cfg_dir = "/usr/share/evilginx/config/"
			if _, err := os.Stat(*cfg_dir); os.IsNotExist(err) {
				*cfg_dir = joinPath(exe_dir, "./config")
			}
		}
	...
	gophish_db_path := joinPath(exe_dir, "../gophish/gophish.db")
	err = database.OpenGoPhishDB(gophish_db_path)
	if err != nil {
		log.Fatal("GophishDB: %v", err)
		return
	}
